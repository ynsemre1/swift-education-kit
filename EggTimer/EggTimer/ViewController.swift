    ...
    func startCountdown(_ countdownSeconds: Int) {
        ...
        countdownTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { timer in
            if remainingSeconds > 0 {
                ...
                remainingSeconds -= 1
                // Calculate progress as a Float value between 0.0 and 1.0
                let progress = Float(countdownSeconds - remainingSeconds) / Float(countdownSeconds)
                self.progressBar.progress = progress // Set the progress
            } else {
                timer.invalidate() // Stop the countdown when it reaches zero
                self.titleLabel.text = "Done!.."
                self.progressBar.progress = 0.0 // Set the progress
            }
        }
    }
}
